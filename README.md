...
P.S.: I tried to run the above sample not only on Windows, but also on native Linux, MacOS (BigSur on Apple Silicon) and even on a RaspberryPI 4 with an experimental 64Bit Version of RaspiOS. The haskell tooling for the aarch64 architecture is available, although it could be tricky to get the current pieces and make them work together. Especially the stack tool for aarch64 is tricky to get. For Linux you can find unofficial binaries by googling for them, on M1 based MacOS I built it from sources using the available cabal tool. 
